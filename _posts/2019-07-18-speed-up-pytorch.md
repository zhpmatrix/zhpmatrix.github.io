...
>最近组里同学在内网博客上放了一篇博客，如何使用Tensorflow训练大模型。因此，想PyTorch下应该也有对应技术，于是有了这篇博客。大模型的训练方法，同样适用于一般场景。需要解决的问题包括计算和存储，容易忽略的时间开销在IO。为了得到一个好用的模型，从数据到模型是一个系统工程，即使相关优化技术都用上，也可能比不用任何优化技术的开发周期长的多，比如模型设计不合理，数据不合适，指标有误，代码Bug，系统环境配置等，但是并不妨碍在极限场景下对于该类技术的需求。
